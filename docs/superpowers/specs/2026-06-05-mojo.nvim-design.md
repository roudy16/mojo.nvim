...
- `lua/mojo/filetype.lua` registers `.mojo` and `🔥` extension detection and activates the project environment on file open.
- `lua/mojo/env.lua` detects Pixi and venv project environments.
- `lua/mojo/treesitter.lua` manages parser lifecycle (register, compile, staleness); pure state — no side effects.
