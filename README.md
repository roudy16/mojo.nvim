...
- **Statusline (lualine.nvim)** — Shows `󰈸 env version` with separate colors for icon and text. Customize via `statusline` config. For non-lualine statuslines, use `require("mojo.status").MojoVersion()`.
